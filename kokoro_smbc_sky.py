...
os.makedirs("segments_smbc_sky", exist_ok=True)

final_audio = AudioSegment.silent(duration=0)

# Per-segment speed override — timestamp: speed
segment_speed_overrides = {
    "00:01:24": 1.35,   # AI Insights — extra text added, speed up to fit ~1:37
}

for idx, (time_line, text_line) in enumerate(entries):
    clean_text = text_line.replace("—", ", ").replace("–", ", ").replace("'", "'").replace("'", "'")
    speed = segment_speed_overrides.get(time_line, 1.01)
    print(f"\n[{idx+1}/{len(entries)}] {time_line} (speed={speed})")
    ...
    try:
        ...
        seg.export(f"segments_smbc_sky/seg_{idx+1:02d}_{time_line.replace(':', '-')}.mp3", format="mp3")
...
final_audio.export("final_audio_smbc_sky.mp3", format="mp3")
mins = len(final_audio) // 60000
secs = (len(final_audio) % 60000) // 1000
print(f"\n✅ Done! final_audio_smbc_sky.mp3 — {mins}:{secs:02d}")
